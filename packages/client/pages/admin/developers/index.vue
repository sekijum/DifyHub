<template>
    <v-container fluid>
      ...
      <v-card variant="outlined">
        <v-data-table-server
          ...
          no-data-text="条件に一致する開発者が見つかりません"
          must-sort
          fixed-header
        ...
        >
          ...
          <template v-slot:item.appsCount="{ item }">
            <span class="text-no-wrap">{{ item._count?.createdApps || 0 }}</span>
          </template>
  ...
  // ページネーション結果型
  interface PaginatedDevelopersResult {
    data: Developer[];
    total: number;
  }
  ...
  const headers: ReadonlyHeaders = [
    ...
    { title: '公開アプリ数', key: 'appsCount', align: 'center', width: 100, sortable: false },
  ...
  const fetchDevelopers = async (): Promise<void> => {
    ...
    try {
      // API呼び出し
      const { data: response } = await $api.get<PaginatedDevelopersResult>('/admin/developers', {
        params,
      });
      ...
      showError(error.response?.data?.message || '開発者情報の取得に失敗しました');
      developers.value = [];
      totalItems.value = 0;
    } finally {
      loading.value = false;
